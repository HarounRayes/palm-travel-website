...
@if(Auth::guard('admin')->user()->hasRole('super-admin') || Auth::guard('admin')->user()->hasAllPermissions(['partners.edit.en','partners.create.ar']))
    <?php
    $active_ar = '';
    $active_en = 'fade show active';
    $ar_tab = $en_tab = true;
    ?>
@elseif (Auth::guard('admin')->user()->hasPermissionTo('partners.edit.en'))
    <?php
    $active_ar = '';
    $active_en = 'fade show active';
    $ar_tab = false; $en_tab = true;
    ?>
@elseif (Auth::guard('admin')->user()->hasPermissionTo('partners.edit.ar'))
    <?php
    $active_en = '';
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                ...
                <form id="demo-form2" method="POST" action="{{ route('admin.partners.update', $partner->id) }}"
                        ...
                        <div class="card card-primary card-outline card-outline-tabs">
                            ...
                            <div class="card-body">
                                <div class="tab-content" id="custom-tabs-four-tabContent">
                                    @if (Auth::guard('admin')->user()->hasPermissionTo('partners.edit.en'))
                                    ...
                                    @endif
                                    @if (Auth::guard('admin')->user()->hasPermissionTo('partners.edit.ar'))
                                ...
                                <div class="form-group">
                                    ...
                                    @if(isset($partner->image))
                                        <img class="prev-img" width="150"
                                             src="{{ url('storage/app/public/images/partner/'.$partner->image) }}"/>
                                    @endif
                                </div>
                            </div>
                        </div>
